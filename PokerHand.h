...
class PokerHand
{
	...
	public:
		...
		static PokerHandType GetHandFromString(const char* apsz_HandName){
			...
			if(!strcmpi(apsz_HandName,"HIGH CARD"))return(POKER_HIGHCARD);
			if(!strcmpi(apsz_HandName,"PAIR"))return(POKER_PAIR);
			if(!strcmpi(apsz_HandName,"TWO PAIRS"))return(POKER_TWO_PAIRS);
			if(!strcmpi(apsz_HandName,"THREE OF A KIND"))return(POKER_THREE_OF_A_KIND);
			if(!strcmpi(apsz_HandName,"STRAIGHT"))return(POKER_STRAIGHT);
			if(!strcmpi(apsz_HandName,"FLUSH"))return(POKER_FLUSH);
			if(!strcmpi(apsz_HandName,"FULL HOUSE"))return(POKER_FULL_HOUSE);
			if(!strcmpi(apsz_HandName,"FOUR OF A KIND"))return(POKER_FOUR_OF_A_KIND);
			if(!strcmpi(apsz_HandName,"STRAIGHT FLUSH"))return(POKER_STRAIGHT_FLUSH);
			if(!strcmpi(apsz_HandName,"ROYAL FLUSH"))return(POKER_ROYAL_FLUSH);
		...
		const char* HandName(void){
			return(PokerHand::GetHandName(me_Hand));
		}
		PokerHandType Hand(void){
			return(me_Hand);
		}
